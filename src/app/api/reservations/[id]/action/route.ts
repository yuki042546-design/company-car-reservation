...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  const { data: vehicleRow } = await supabase
    .from("vehicles")
    .select("track_mileage")
    .eq("id", existing.vehicleId)
    .maybeSingle();
  const trackMileage = vehicleRow?.track_mileage ?? true;

  if (body.action === "depart") {
    if (trackMileage) {
      if (
        typeof body.departureOdometer !== "number" ||
        !Number.isFinite(body.departureOdometer) ||
        body.departureOdometer < 0
      ) {
    ...
    await supabase.from("vehicle_usage_records").insert({
      ...
      departure_odometer: trackMileage ? body.departureOdometer : null,
  ...
  if (body.action === "return") {
    ...
    await supabase
      ...
      .update({
        ...
        return_odometer: trackMileage ? body.returnOdometer : null,
