...
- `/admin/vehicles` の各車両に「走行距離を記録しない設定にする」ボタンがある
  （`vehicles.track_mileage`）。私用兼用の車両など、走行距離を記録したくない
  車両をオフにすると、出発・返却画面で走行距離の入力欄自体が表示されなくなる。
- オフにした後の出発・返却記録（`vehicle_usage_records`）は走行距離が`null`
  で保存される。利用履歴の「走行距離」列には「-」と表示される。稼働率は
  予約時間ベースの算出のため、この設定による影響を受けない。
- 新規追加時は追加フォームのチェックボックスで指定できる（既定はオン）。
