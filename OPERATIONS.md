...
1. `/admin` にアクセス（`system_admin`権限が必要）
2. 「ユーザー管理」セクションの「新しいユーザーを招待」にメールアドレス・初期ロール・
   部署（任意）を入力し「招待を送信」
3. 対象者に招待メールが届く（Supabase Authが自動送信）。メール内リンクから
   パスワードを設定するとログインできるようになる
4. `ALLOWED_EMAIL_DOMAINS` に含まれないドメインのメールアドレスは招待できない
   （環境変数を確認すること）

## 社員を無効化する

1. `/admin` の「ユーザー管理」で対象ユーザーの「無効化」ボタンを押す
2. 無効化されたユーザーはログイン後すぐに操作を拒否される（`active=false`）
3. 過去の予約データは削除されず、そのまま残る

## 権限を変更する

1. `/admin` の「ユーザー管理」で対象ユーザーのロールをプルダウンから選択
2. `system_admin`のみがこの操作を行える

## 車両情報を変更する

- 現時点では車両情報の編集UIは未実装（`vehicles`テーブル・APIはあるが専用画面がない）。
  当面はSupabaseのTable Editorから直接編集するか、`PATCH /api/vehicles/[id]`を
  `vehicle_manager`以上のアカウントでログインした状態から直接呼び出すこと。
...
- `/admin` の「監査ログ」セクションで、実行者メール・操作種別・対象・理由・日時を確認できる
...
3. Supabaseダッシュボードで「Enable email signups」が無効化されていることを確認
4. Vercelの環境変数に `NEXT_PUBLIC_SUPABASE_ANON_KEY` / `ALLOWED_EMAIL_DOMAINS` /
   `CRON_SECRET` を追加し、旧`ADMIN_PASSWORD`/`ADMIN_SESSION_SECRET`は削除する
5. デプロイ後、`system_admin`アカウントを最低1人招待し、ログインできることを確認
6. 一般社員アカウントでログインし、予約作成・変更・キャンセルが正しく制御されることを確認
