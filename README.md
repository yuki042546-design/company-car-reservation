...
- 今日の予約は「リスト」表示と「ガントチャート」表示を切り替えて確認可能
- 予約変更（全社員が利用可）
...
このアプリはブラウザから直接 Supabase を呼び出さず、**必ず Next.js の API ルート（サーバー側・service role key）を経由**します。そのため両テーブルとも RLS を有効化した上でポリシーを一切作成せず、anon キーによる直接アクセスを完全にブロックしています。
...
src/
  app/
    page.tsx                       トップページ（今日/今週の予約、新規予約ボタン）
  ...
  components/
    ...
    Header.tsx                     共通ヘッダー
  lib/
    types.ts                       型定義（Reservation, Employee など）
    ...
    dateUtils.ts                   日時フォーマット・今日/今週の範囲計算（Asia/Tokyo固定）
supabase/
